...
### 1. Orchestrator + Subagents (Divide & Conquer)
For any non-trivial task, use the Agent tool to delegate to specialized subagents with clean context windows. The orchestrator only coordinates — it never drowns in implementation details.

**When to spawn subagents:**
- Exploring an unfamiliar codebase before implementing
- Running independent tasks in parallel (tests, linting, research)
- Any task that would require reading 5+ files

### 2. Skills as Lazy-Loaded Context
Load skills only when needed. This file is the router — skills are the modules.

**Routing table:**
| Task | Skill to load |
|---|---|
| React components, hooks, RSC | `react` |
| React Native / Expo | `expo`, `react-native-expo`, `react-native-best-practices` |
| RN styling / navigation / animations | `react-native-design` |
| Mobile UX patterns | `mobile-design` |
| Tailwind CSS v4 | `tailwind` |
| TypeScript config / types | `typescript` |
| TanStack Query | `tanstack-query` |
| React Hook Form | `react-hook-form` |
| Astro framework | `astro-framework` |
| Clean Architecture / system design | `clean-architecture` |
| Git commit / push / PR | `commit-commands:commit` or `commit-commands:commit-push-pr` |
| Frontend UI design (new screens) | `frontend-design:frontend-design` |
| Spec-Driven Development | `sdd` (see below) |

### 3. Engram — Persistent Memory
Memory is ALWAYS active via the `engram` plugin. Proactively save:
...
Before implementing any significant feature, follow this chain:
1. **EXPLORE** — Subagent reads relevant code, no changes
2. **PROPOSE** — Draft approach options (min 2), present tradeoffs
3. **SPECIFY** — Write an OpenSpec (see `/sdd-spec` skill)
4. **DESIGN** — Review spec with user, confirm before coding
5. **IMPLEMENT** — Code only after spec is approved

Invoke with `/sdd` or ask for "spec-driven" mode.

### 5. Tony Stark Model
Claude executes. The user decides architecture. When facing an important architectural fork, always pause and present options — never choose silently.

---

## Conventions

- Responses: concise, no filler, no trailing summaries
- Code: no speculative abstractions, no defensive error handling for impossible cases
- Tests: match the project's existing test strategy
